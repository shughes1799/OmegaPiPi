...
void MCOpt::SlaveBegin(TTree * /*tree*/)
{
   ...
   //MC Optimisation Histograms
   h1_proton_mc_theta  = new TH1F("h1_proton_mc_theta","Proton Generation theta; Degrees",220,-20,200);
   h1_pip1_mc_theta  = new TH1F("h1_pip1_mc_theta","PiP1 Generation theta; Degrees",220,-20,200);
   h1_pip2_mc_theta  = new TH1F("h1_pip2_mc_theta","PiP2 Generation theta; Degrees",220,-20,200);
   h1_pim1_mc_theta  = new TH1F("h1_pim1_mc_theta","PiM1 Generation theta; Degrees",220,-20,200);
   h1_pim2_mc_theta  = new TH1F("h1_pim2_mc_theta","PiM2 Generation theta; Degrees",220,-20,200);



   h2_proton_mctheta_vs_reconp = new TH2F("h2_proton_mctheta_vs_reconp","Proton mc theta vs recon p; Degrees",60,0,60,50,0,1.5);
   h2_pip_mctheta_vs_reconp  = new TH2F("h2_pip_mctheta_vs_reconp","PiP mc theta vs recon p; Degrees",180,0,180,50,0,1.5);
   h2_pim_mctheta_vs_reconp  = new TH2F("h2_pim_mctheta_vs_reconp","PiM mc theta vs recon p; Degrees",180,0,180,50,0,1.5);
   ...
   //MC Opt Histograms
   fOutput->Add(h1_proton_mc_theta);
   fOutput->Add(h1_pip1_mc_theta);
   fOutput->Add(h1_pip2_mc_theta);
   fOutput->Add(h1_pim1_mc_theta);
   fOutput->Add(h1_pim2_mc_theta);

   fOutput->Add(h2_proton_mctheta_vs_reconp);
   fOutput->Add(h2_pip_mctheta_vs_reconp);
   fOutput->Add(h2_pim_mctheta_vs_reconp);
...
Bool_t MCOpt::Process(Long64_t entry)
{
   // The Process() function is called for each entry in the tree (or possibly
   // keyed object in the case of PROOF) to be processed. The entry argument
   // specifies which entry in the currently loaded tree is to be processed.
   // It can be passed to either MCOpt::GetEntry() or TBranch::GetEntry()
  ...
  //   if(mcid[ii] == 2212){
  //     h1_proton_mc_theta->Fill(mctheta[ii]); 
      
  //     for(int jj=0;jj<gpart;jj++){
  // 	if(id[jj] == 2212){
  // 	  h2_proton_mctheta_vs_reconp->Fill(mctheta[ii],p[jj]);
  // 	}
  //     }
  //   } //end of proton bit

  //    if(mcid[ii] == 211){
  //      h1_pip_mc_theta->Fill(mctheta[ii]);
      
  //      for(int jj=0;jj<gpart;jj++){
  //      	 if(id[jj] == 211){
  //      	   h2_pip_mctheta_vs_reconp->Fill(mctheta[ii],p[jj]);
  //      		 }
  //      	 }

  //    }
  //     if(mcid[ii] == -211){
  //   	h1_pim_mc_theta->Fill(mctheta[ii]);

  // 	for(int jj=0;jj<gpart;jj++){
  // 	  if(id[jj] == -211){
  // 	    h2_pim_mctheta_vs_reconp->Fill(mctheta[ii],p[jj]);
  // 	  }
  // 	}
 ...
 for(int ii=0;ii<gpart;ii++){

    // if(id[ii] != 2212 && id[ii] != 211 && id[ii] != -211){
 ...
 // for(int jj=0;jj<20;jj++){
   ...
   // if(mcid[jj] == 2212){
       h1_proton_mc_theta->Fill(mctheta[0]); 
       //  }
 // if(mcid[jj] == 211){
       h1_pip1_mc_theta->Fill(mctheta[1]);
       // }
       // if(mcid[jj] == -211){
       h1_pim1_mc_theta->Fill(mctheta[2]);
       // }
       h1_pip2_mc_theta->Fill(mctheta[4]);
       h1_pim2_mc_theta->Fill(mctheta[5]);
...
void MCOpt::Terminate()
{
   // The Terminate() function is the last function to be called during
   // a query. It always runs on the client, it can be used to present
   // the results graphically or save the results to file.

  TFile file_out("MCOpt_output.root","recreate");
...
Bool_t MCOpt::MakeBeam(Float_t Tmid,Float_t Tcut){
