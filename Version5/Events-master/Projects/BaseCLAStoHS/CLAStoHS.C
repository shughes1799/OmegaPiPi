...
void CLAStoHS::SlaveBegin(TTree * /*tree*/)
{
  ...
  THSOutput::HSSlaveBegin(fInput,fOutput);
   ...
   fOutTree->Branch("MesonXTest",&fMesonXTest,buff,split);
}
...
Bool_t CLAStoHS::Process(Long64_t entry)
{
   ...
   *fMissing=gTarget+fHSgamma->P4();
   ...
   // *fMesonXTest=gTarget+fHSgamma->P4(); //Only Valid when proton is the 3rd particles e.g. Pi+Pi-Proton.
   // *fMesonXTest-=fDetParticle[2]->P4();
