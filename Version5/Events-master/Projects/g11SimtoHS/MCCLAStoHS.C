...
Bool_t MCCLAStoHS::Process(Long64_t entry)
{
  ...
  /// if(!IsGoodEvent()) return kTRUE; //not the event we want so exit
   ...
   //Selecting only the particles that decayed from the omega.
   
   // for(Uint_t i=0;i<mcnentr;i++){
    
   // cout << "MC Particle ID's " << mcid[0] << " " << mcid[1] << " " << mcid[2] << " "<< mcid[3] << " "<< mcid[4] << " "<< mcid[5] << " "<< endl;
     // }
