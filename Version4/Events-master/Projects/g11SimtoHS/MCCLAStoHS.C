...
void MCCLAStoHS::SlaveBegin(TTree * /*tree*/)
{
  ...
  THSOutput::HSSlaveBegin(fInput,fOutput);
   ...
   fProton=new TLorentzVector();
   fPiP=new TLorentzVector();
   fPiM=new TLorentzVector();
   fOutTree->Branch("beam",&fHSgamma,buff,split);
   fOutTree->Branch("miss",&fMissing,buff,split);
   fOutTree->Branch("MCTruth",&fMCParticle,buff);
   fOutTree->Branch("Proton",&fProton,buff,split);
   fOutTree->Branch("PiP",&fPiP,buff,split);
   fOutTree->Branch("PiM",&fPiM,buff,split);

   fOutTree->Branch("PhiPiP1",&PhiPiP1,"PhiPiP1/D");
   fOutTree->Branch("PhiPiP2",&PhiPiP2,"PhiPiP2/D");
   fOutTree->Branch("PhiPiM1",&PhiPiM1,"PhiPiM1/D");
   fOutTree->Branch("PhiPiM2",&PhiPiM2,"PhiPiM2/D");
   fOutTree->Branch("PhiProton",&PhiProton,"PhiProton/D");

   fOutTree->Branch("ThetaPiP1",&ThetaPiP1,"ThetaPiP1/D");
   fOutTree->Branch("ThetaPiP2",&ThetaPiP2,"ThetaPiP2/D");
   fOutTree->Branch("ThetaPiM1",&ThetaPiM1,"ThetaPiM1/D");
   fOutTree->Branch("ThetaPiM2",&ThetaPiM2,"ThetaPiM2/D");
...
Bool_t MCCLAStoHS::Process(Long64_t entry)
{
   ...
   *fMissing=gTarget+fHSgamma->P4();
   ...
   Double_t Mmiss = fMissing->M();

   if(Mmiss <= 0.68 || Mmiss >= 0.88) return kTRUE;

   *fPiM=fDetParticle[0]->P4();
   *fPiP=fDetParticle[1]->P4();
   *fProton=fDetParticle[2]->P4();

   for(Int_t mc=0;mc<6;mc++){
     if(mc==0){
       PhiProton = mcphi[mc];
       ThetaProton = mctheta[mc];
     }
     if(mc==1){
       PhiPiP1 = mcphi[mc];
       ThetaPiP1 = mctheta[mc];
     }
     if(mc==2){
       PhiPiM1 = mcphi[mc];
       ThetaPiM1 = mctheta[mc];
     }
     if(mc==4){
       PhiPiP2 = mcphi[mc];
       ThetaPiP2 = mctheta[mc];
     }
     if(mc==5){
       PhiPiM2 = mcphi[mc];
       ThetaPiM2 = mctheta[mc];
     }
   }
   
   //PhiProton = mcphi[0];
   //ThetaProton = mctheta[0];

   // cout << "ParticlesIDs " << mcid[0] << " " << mcid[1] << " " << mcid[2] << " " << mcid[3] << " " << mcid[4] << " " << mcid[5]  << endl;

   cout << "Properties of the Proton " << mcphi[0] << " " << mctheta[0] << endl;
   cout << "Properties of the Proton " << PhiProton << " " << ThetaProton << endl;
...
void MCCLAStoHS::GetEventPartBranches(Int_t evi){
  ...
  b_mcnpart->GetEvent(evi);   //!
  b_mcw->GetEvent(evi);   //!
  b_mcst->GetEvent(evi);   //!
  b_mcid->GetEvent(evi);   //!
  b_mcpid->GetEvent(evi);   //!
